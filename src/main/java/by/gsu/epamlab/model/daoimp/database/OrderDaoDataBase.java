...
public class OrderDaoDataBase implements IOrderDao {
    ...
    private static final String SELECT_BUSY_TICKET_BY_ID = "SELECT id, userId, eventId, ticket, date FROM orders WHERE eventId = ?";
    private static final String SELECT_TICKET_BY_USER_ID = "SELECT id, userId, eventId, ticket, date FROM orders WHERE eventId = ? AND userId = ?";
    private static final String SELECT_TICKET_BY_ID_WO_USER = "SELECT id, userId, eventId, ticket, date FROM orders WHERE eventId = ? AND userId != ?";
    private static final String SELECT_ALL_ACTUALY_ARDER_BY_USER_ID = "SELECT orders.id, orders.userId, orders.eventId, orders.ticket, orders.date, " +
            "events.name, events.date FROM orders, events WHERE orders.userId = ? AND orders.eventId = events.id";
    ...
    public List<Order> getOrderByUserId(int userId, Status status) {
        ...
        try(Connection connection = ConnectionManager.getConnection();
            PreparedStatement prstm =
                    connection.prepareStatement(SELECT_ALL_ACTUALY_ARDER_BY_USER_ID)) {
                    ...
                    switch (status) {
                        case OLD:
                            if (new Date().after(eventDate)) {
                                order = new Order(orderId, orderUserId,
                                        orderEventId, orderDate, ticket);
                                list.add(order);
                            }
                            break;
                        case NEW:
    ...
    public List<Order> getAll() {
            ...
            try(Connection connection = ConnectionManager.getConnection();
                ...
                while(rs.next()) {
                    ...
                    String ticket = rs.getString(4);
                    Date orderDate = new Date(rs.getTimestamp(5).getTime());
                    Order order = new Order(orderId, orderUserId, orderEventId,
                            orderDate, ticket);
    ...
    public String getAllBusyTicketByEventId(int eventId) {
        ...
        try(Connection connection = ConnectionManager.getConnection();
            PreparedStatement prstm =
                    connection.prepareStatement(SELECT_BUSY_TICKET_BY_ID)) {
    ...
    public String getAllSelectedTicketByUserId(int eventId, int userId) {
        ...
        try(Connection connection = ConnectionManager.getConnection();
            PreparedStatement prstm =
                    connection.prepareStatement(SELECT_TICKET_BY_USER_ID)) {
            prstm.setInt(1, eventId);
    ...
    public String getAllBusyTicketByUserId(int eventId, int userId) {
        ...
        try(Connection connection = ConnectionManager.getConnection();
            PreparedStatement prstm =
                    connection.prepareStatement(SELECT_TICKET_BY_ID_WO_USER)) {
            prstm.setInt(1, eventId);
            prstm.setInt(2, userId);
            try(ResultSet rs = prstm.executeQuery()) {
                while(rs.next()) {
