...
public class BookingController extends AbstractController {
    ...
    private void performGetReq(HttpServletRequest req, HttpServletResponse resp,
        ...
        String seatBusy = AbstractDaoFactory.getDaoFactory(Constant.FACTORY).
                getOrderDao().getAllBusyTicketByEventId(event.getId());
        ...
        if (user != null) {
            seatBusy = AbstractDaoFactory.getDaoFactory(Constant.FACTORY).
                    getOrderDao().getAllBusyTicketByUserId(event.getId(),
                                                           user.getId());
            String seatSelected = AbstractDaoFactory.
    ...
    private void performAjaxReq(HttpServletRequest req, HttpServletResponse resp,
        ...
        try {
            if(!"[]".equals(json)) {
                Order order = new Order(1, user.getId(),
                                        Integer.parseInt(idEvent), new Date(),
                                        json.replace("[", "").
                                                 replace("]", ""));
                AbstractDaoFactory.getDaoFactory(Constant.FACTORY).getOrderDao().
                        insert(order);
                String seatBusy = AbstractDaoFactory.
                        getDaoFactory(Constant.FACTORY).getOrderDao().
                        getAllBusyTicketByUserId(Integer.parseInt(idEvent),
                                                 user.getId());
                String seatSelected = AbstractDaoFactory.
                        getDaoFactory(Constant.FACTORY).getOrderDao().
                        getAllSelectedTicketByUserId(Integer.parseInt(idEvent),
                                                     user.getId());
